...
# harmonising
smk_harm <- harmonise_data(exposure_dat = bmi_data, outcome_dat = smk_data)
nrow(smk_harm)   # 1851
...
colnames(int) <- c("rsid", "chromosome", "position", "effect allele", "non-effect allele",
                   ...
                   "BMI standard error", "Lifetime-smoking effect size", "Lifetime-smoking standard error")

write.csv(int, "/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/instruments/instrument.bmi->LtSmk.csv", row.names = FALSE)
...
p1
pdf("/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/scatterplots/bmi->LtSmk.scatter.pdf")
p1[[1]]
dev.off()
