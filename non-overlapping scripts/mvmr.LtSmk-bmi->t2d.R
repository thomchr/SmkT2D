...
length(which(t2d$Chr.Position %in% smk_data$key == TRUE)) #7631132
...
smk_sig <- temp_merge[,c("key","chr.exposure.x","pos.exposure.x","SNP.x", 
                         ...
                         "beta.exposure.x","se.exposure.x","samplesize.exposure","exposure.x",
...
bmi_sig <- temp_merge[,c("key","chr.exposure.y","pos.exposure.y","SNP.y", 
                         ...
                         "beta.exposure.y","se.exposure.y","samplesize.exposure","exposure.y",
...
colnames(merge.sig) <- c("key","SNP","effect_allele.exposure","other_allele.exposure",
                         ...
                         "se.outcome","pval.outcome","samplesize.outcome","chr.outcome","pos.outcome",
                         "outcome","mr_keep.outcome","pval_origin.outcome","chr.exposure","pos.exposure",
                         "pval.exposure","se.exposure","samplesize.exposure","exposure","mr_keep.exposure",
                         ...
                         "ambiguous.y","id.outcome.y","se.outcome.y","pval.outcome.y","samplesize.outcome.y",
                         "chr.outcome.y","pos.outcome.y","outcome.y","mr_keep.outcome.y","pval_origin.outcome.y",
                         "chr.exposure.y","pos.exposure.y","pval.exposure.y","se.exposure.y","samplesize.exposure.y",
                         "exposure.y","mr_keep.exposure.y","pval_origin.exposure.y","id.exposure.y","action.y","mr_keep.y")
...
write.csv(int, "/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/instruments/instrument.LtSmk_bmi->t2d.csv", row.names = FALSE)
