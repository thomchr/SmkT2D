...
colnames(merge.sig) <- c("key","SNP","effect_allele.exposure","other_allele.exposure","effect_allele.outcome",
                         ...
                         "pval.outcome","outcome","mr_keep.outcome","pval_origin.outcome","chr.exposure",
                         "pos.exposure","se.exposure","pval.exposure","exposure","mr_keep.exposure",
                         "pval_origin.exposure","id.exposure","action","mr_keep","samplesize.outcome",
                         "SNP.y","effect_allele.exposure.y","other_allele.exposure.y","effect_allele.outcome.y",
                         "other_allele.outcome.y","beta.exposure.y","beta.outcome.y","eaf.exposure.y",
                         "eaf.outcome.y","remove.y","palindromic.y","ambiguous.y","id.outcome.y",
                         "chr.outcome.y","pos.outcome.y","se.outcome.y","pval.outcome.y","outcome.y",
                         "mr_keep.outcome.y","pval_origin.outcome.y","se.exposure.y","pval.exposure.y",
                         "samplesize.exposure","chr.exposure.y","pos.exposure.y","exposure.y",
                         "mr_keep.exposure.y","pval_origin.exposure.y","id.exposure.y","action.y",
                         "mr_keep.y","samplesize.outcome.y")
...
colnames(int) <- c("rsid", "chromosome", "position", "effect allele", "non-effect allele",
                   "effect allele frequency", "liftime-smoking effect size",
                   "liftime-smoking standard error", "BMI effect size", "BMI standard error",
                   "CAD effect size", "CAD standard error")

write.csv(int, "/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/instruments/instrument.LtSmk_bmi->cad.csv", row.names = FALSE)

## filter by direction
# filter by directions: 
combined.flt <- combined[(  (combined$beta.insulin>0) & (combined$BETA.hdl<0) & (combined$BETA.tg>0) ) |
                           (  (combined$beta.insulin<0) & (combined$BETA.hdl>0) & (combined$BETA.tg<0)   ),]
nrow(combined.flt) # 638
