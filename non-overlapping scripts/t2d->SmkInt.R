...
length(which(t2d.sig$Chr.Position %in% smk_data$key == TRUE)) #1715
t2d.sig <- t2d.sig[t2d.sig$Chr.Position %in% smk_data$key,]

t2d.sig$chr <- str_split_fixed(t2d.sig$Chr.Position, ":", 2)[,1]
t2d.sig$pos <- str_split_fixed(t2d.sig$Chr.Position, ":", 2)[,2]
...
colnames(int) <- c("rsid", "chromosome", "position", "effect allele", "non-effect allele",
                   "effect allele frequency", "T2D effect size",
                   "T2D standard error", "smoking-initiation effect size", "smoking-initiation standard error")

write.csv(int, "/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/instruments/instrument.t2d->SmkInt.csv", row.names = FALSE)
...
p1
pdf("/Users/dr/Desktop/MR.SMK-T2D/non_overlap_analyses/scatterplots/t2d->SmkInt.scatter.pdf")
p1[[1]]
dev.off()
