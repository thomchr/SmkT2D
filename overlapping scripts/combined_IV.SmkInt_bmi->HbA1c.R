...
nrow(bmi_red) # 2310060

# harmonising by T2D data
smk_harm <- harmonise_data(exposure_dat = smk_red, outcome_dat = hb_data)
nrow(smk_harm) # 2292784

bmi_harm <- harmonise_data(exposure_dat = bmi_red, outcome_dat = hb_data)
nrow(bmi_harm) # 2291981
...
colnames(merge.sig) <- c("key","SNP","effect_allele.exposure","other_allele.exposure",
                         ...
                         "pos.exposure","pval.exposure","se.exposure","samplesize.exposure","exposure",
                         "mr_keep.exposure","pval_origin.exposure","id.exposure","action","mr_keep",
                         "samplesize.outcome","SNP.y","effect_allele.exposure.y","other_allele.exposure.y",
...
colnames(int) <- c("rsid", "chromosome", "position", "effect allele", "non-effect allele",
                   ...
                   "HbA1c effect size", "HbA1c standard error")

write.csv(int, "/Users/dr/Desktop/MR.SMK-T2D/better_instrumnet_mvmr/instruments/combined_IV.SmkInt_bmi->HbA1c.instrument.csv", row.names = FALSE)
